...
public class GlideBlurTransform extends BitmapTransformation {
    ...
    public static class Builder {
        ...
        public GlideBlurTransform build() {
            return new GlideBlurTransform(mContext, mRadius, mColor);
        }
     ...
     */
    public GlideBlurTransform(Context context, @FloatRange(from = 0.0f) float radius, int color) {
        super(context);
        mContext = context;
     ...
     * @param radius The blur's radius.
     */
    public GlideBlurTransform(Context context, @FloatRange(from = 0.0f) float radius) {
...
//        toTransform.recycle();  <--- Just for tips. by Ligboy
     ...
     * <p/>
     * <p>
     * If this transformation does not affect the data that will be stored in cache, returning an list_empty_icon string here
     * is acceptable.
     * </p>
     *
